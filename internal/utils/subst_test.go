...
var substCases = []SubstTest{
	...
	},
	{
		...
		Args:   []string{"dev=jane"},
		Result: `{"name":"service-jane"}`,
	},
	{
		Yaml:   `{"name":"@{team}-service-@{dev}"}`,
		Args:   []string{"team=gorillas", "dev=jane"},
		Result: `{"name":"gorillas-service-jane"}`,
	},
	{
		Yaml:   `{"name":"@{team}@{dev}"}`,
		Args:   []string{"team=gorillas", "dev=jane"},
		Result: `{"name":"gorillasjane"}`,
	},
	{
		Yaml:   `{"name":"b@{team}@{dev}e"}`,
		Args:   []string{"team=gorillas", "dev=jane"},
		Result: `{"name":"bgorillasjanee"}`,
	},
}
...
func testLoadUnstructuredTemplate(tc *SubstTest, t *testing.T) {
	...
	template, err := LoadUnstructuredTemplate(f.Name(), *tplVals, false)
	if err == nil && tc.Error {
