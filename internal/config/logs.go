...
func (c *Logs) AddFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&c.Plan, "plan", "", "plan execution ID whose log lines will be displayed")
	cmd.Flags().StringVar(&c.Step, "step", "", "step ID (used with --plan for step-level logs)")
