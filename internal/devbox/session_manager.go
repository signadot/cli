...
func NewSessionManager(log *slog.Logger, ciConfig *config.ConnectInvocationConfig, shutdownCh chan struct{}) (*SessionManager, error) {
	...
	apiClient, err := createAPIClient(ciConfig)
	if err != nil {
...
func createAPIClient(ciConfig *config.ConnectInvocationConfig) (*client.SignadotAPI, error) {
...
func (dsm *SessionManager) renewSession(ctx context.Context) {
	...
	// Recreate API client if needed (in case auth changed)
	apiClient, err := createAPIClient(dsm.ciConfig)
	if err != nil {
